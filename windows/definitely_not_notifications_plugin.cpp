#include "include/definitely_not_notifications/definitely_not_notifications_plugin.h"
...
class CustomToastHandler : public IWinToastHandler {
 public:
  ...
  void toastActivated() const {
    ...
    channel->InvokeMethod("onNotificationClicked",
  ...
  void toastActivated(int actionIndex) const {
    ...
    channel->InvokeMethod("onNotificationAction",
                          std::make_unique<flutter::EncodableValue>(args));
  }
  ...
  void toastDismissed(WinToastDismissalReason state) const {
    ...
    channel->InvokeMethod("onNotificationClosed",
                          std::make_unique<flutter::EncodableValue>(args));
  }
...
class DefinitelyNotNotificationsPlugin : public flutter::Plugin {
 public:
  static void RegisterWithRegistrar(flutter::PluginRegistrarWindows* registrar);

  DefinitelyNotNotificationsPlugin();

  virtual ~DefinitelyNotNotificationsPlugin();
 ...
 private:
  ...
  HWND DefinitelyNotNotificationsPlugin::GetMainWindow();
  void DefinitelyNotNotificationsPlugin::Setup(
      const flutter::MethodCall<flutter::EncodableValue>& method_call,
      std::unique_ptr<flutter::MethodResult<flutter::EncodableValue>> result);
  void DefinitelyNotNotificationsPlugin::Notify(
      const flutter::MethodCall<flutter::EncodableValue>& method_call,
      std::unique_ptr<flutter::MethodResult<flutter::EncodableValue>> result);
...
// static
void DefinitelyNotNotificationsPlugin::RegisterWithRegistrar(
    flutter::PluginRegistrarWindows* registrar) {
  channel = std::make_unique<flutter::MethodChannel<flutter::EncodableValue>>(
      registrar->messenger(), "local_notifier",
      &flutter::StandardMethodCodec::GetInstance());

  auto plugin = std::make_unique<DefinitelyNotNotificationsPlugin>();
...
DefinitelyNotNotificationsPlugin::DefinitelyNotNotificationsPlugin() {}
...
void DefinitelyNotNotificationsPlugin::Notify(
  ...
  std::string icon =
      std::get<std::string>(args.at(flutter::EncodableValue("icon")));
  std::string hero =
      std::get<std::string>(args.at(flutter::EncodableValue("hero")));
  bool isCircle =
      std::get<std::string>(args.at(flutter::EncodableValue("isCircle"))) == "true";
  bool isInline =
      std::get<std::string>(args.at(flutter::EncodableValue("isInline"))) == "true";

  flutter::EncodableList actions = std::get<flutter::EncodableList>(
      args.at(flutter::EncodableValue("actions")));

  WinToastTemplate toast = WinToastTemplate(WinToastTemplate::HeroImageAndImageAndText02);

  toast.setFirstLine(converter.from_bytes(title));
  toast.setSecondLine(converter.from_bytes(body));
  if (!icon.empty()) {
    toast.setImagePath(converter.from_bytes(icon),isCircle ? WinToastTemplate::CropHint::Circle :  WinToastTemplate::CropHint::Square);

  }
  if(!hero.empty()){
    toast.setHeroImagePath(converter.from_bytes(hero), isInline);
  }
  ...
  for (flutter::EncodableValue action_value : actions) {
    ...
    toast.addInputField(converter.from_bytes(action_text));
  }
...
void DefinitelyNotNotificationsPlugin::HandleMethodCall(
    const flutter::MethodCall<flutter::EncodableValue>& method_call,
    std::unique_ptr<flutter::MethodResult<flutter::EncodableValue>> result) {
...
void DefinitelyNotNotificationsPluginRegisterWithRegistrar(
    FlutterDesktopPluginRegistrarRef registrar) {
  DefinitelyNotNotificationsPlugin::RegisterWithRegistrar(
